...
본 문서는 [Solution05.java](file:///Users/morgan/Documents/workspace/260626_ex/src/Solution05.java) 실습 예제 코드를 기반으로 Java의 입출력 스트림 중 **바이트 스트림(Byte Stream)**의 개념과 파일 읽기/쓰기 구현법, 그리고 발생할 수 있는 인코딩 주의사항을 정리합니다.
...
```
* **동작 흐름**: `String.getBytes(StandardCharsets.UTF_8)` 메서드를 사용해 지정된 인코딩 형식의 byte 배열로 쪼갠 뒤, `FileOutputStream` 스트림에 기록합니다.
...
* **현상**: 영문(ASCII) 데이터인 `"William"`은 1바이트당 1문자에 대응되므로 정상 출력됩니다. 하지만 **한글(UTF-8 기준 자당 3바이트)** 데이터인 `"반갑습니다"` 등은 1바이트씩 쪼개진 상태에서 강제로 2바이트 문자타입인 `(char)`로 캐스팅되므로 글자가 깨지게 됩니다.
